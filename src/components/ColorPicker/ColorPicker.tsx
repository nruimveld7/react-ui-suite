...
export const ColorPicker = React.forwardRef<HTMLInputElement, ColorPickerProps>(function ColorPicker(
  { label, value, defaultValue = "#0ea5e9", onChange, swatches = defaultSwatches, id, className, ...rest },
  forwardedRef
) {
  const normalizedDefault = React.useMemo(() => normalizeHex(defaultValue) ?? "#0EA5E9", [defaultValue]);
  ...
  const [channelValues, setChannelValues] = React.useState<ChannelValues>(() => getChannelValues(resolvedValue, format));
              ...
              <div className="flex flex-wrap gap-2">
                {format === "hex" ? (
                  <label className="flex min-w-[180px] flex-1 items-center gap-2 rounded-xl border border-slate-200 bg-white/60 px-3 py-1.5 font-mono text-sm text-slate-900 focus-within:border-slate-400 dark:border-zinc-700 dark:bg-zinc-900/70 dark:text-zinc-100">
                    <span className="text-base font-semibold text-slate-400 dark:text-zinc-500">#</span>
                    <input
                      type="text"
