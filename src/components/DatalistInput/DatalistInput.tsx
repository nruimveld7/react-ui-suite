...
export const DatalistInput = React.forwardRef<HTMLInputElement, DatalistInputProps>(function DatalistInput(
  { label, description, options, className, id, disabled, ...rest },
  ref
) {
  ...
  const setListRef = (node: HTMLUListElement | null, scrollRef: React.MutableRefObject<HTMLUListElement | null>) => {
            ...
            {({ scrollRef }) => (
              ...
              >
                {filtered.map((opt, index) => (
                  <li key={opt} id={`${listboxId}-option-${index}`} role="option" aria-selected={index === activeIndex}>
                    <button
                      type="button"
